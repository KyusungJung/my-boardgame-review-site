...
필수 검증 사례는 `뒤집어줘`, `하나비`, `이레이저` 검색 시 다음 게임들을 정확한 제목과 메타데이터를 포함한 서버 검색 결과로 반환하는 것이다.
...
- Naver/Ecosia 인덱스 fallback은 일부 게임을 누락했고 검색 링크의 접근성 문구인 `사진`이나 번역된 제목을 원본 제목으로 오인했다. fallback 제목 정제 후에도 검색어와 관련된 결과가 없으면 무관한 결과를 반환하지 않으며, 결과 누락이 확인된 검색군은 검증 카탈로그로 보완한다.
