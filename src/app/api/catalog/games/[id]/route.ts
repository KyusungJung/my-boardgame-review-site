...
import { getGameCatalogMetadata } from "@/lib/game-catalog";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(await getGameCatalogMetadata(id, request.nextUrl.searchParams.get("refresh") === "1", seed));
  } catch (error) {
    console.error("Game catalog detail failed", error);
