...
export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => undefined) as { options?: unknown; refresh?: boolean } | undefined;
  ...
  try {
    ...
    return NextResponse.json({ recommendations: rankMeetingGames(games.map(serializeGame), options, hotness.titles), externalSource: { available: hotness.available, fetchedAt: hotness.fetchedAt, name: "BoardGameGeek Hotness" } });
  } catch (error) {
