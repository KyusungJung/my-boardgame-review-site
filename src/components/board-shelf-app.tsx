...
import { BgtiGuide } from "@/components/bgti-guide";
import { bgtiAxes, bgtiSummary, estimateBgtiWeights, normalizeBgtiWeights } from "@/lib/bgti";
import { hasDisplayableGameDescription, hasUsableGameDescription } from "@/lib/game-description";
import { buildTagRecommendationGroups } from "@/lib/tag-recommendations";
...
const pageMetadata = {
  ...
  bgti: { title: "BGTI", description: "보드게임 성향 지표와 16가지 게이머 타입을 살펴보세요." },
...
const navigationItems = [
  ...
  { key: "bgti", icon: <CompassOutlined />, label: "BGTI" },
...
function GameDescriptionLoading() {
  return <div className="game-description-loading" role="status" aria-label="게임 설명을 불러오는 중입니다."><span /><span /><span /></div>;
}
...
export function BoardShelfApp() {
  ...
  const [collectionStatusFilter, setCollectionStatusFilter] = useState<CollectionGame["status"]>("owned");
  ...
  const filteredCollection = useMemo(() => {
    ...
    return collection.filter((game) => game.status === collectionStatusFilter && (!normalizedQuery || [game.title, game.englishTitle, ...game.tags].some((value) => value.toLocaleLowerCase("ko").includes(normalizedQuery))));
  }, [collection, collectionQuery, collectionStatusFilter]);
  const hasGamesInSelectedCollectionStatus = collection.some((game) => game.status === collectionStatusFilter);
  ...
  const popularTagGroups = useMemo(() => {
    return buildTagRecommendationGroups(collection).map((group) => ({ ...group, count: group.rankedGames.length }));
  }, [collection]);
  ...
  const isViewingGameDescriptionLoading = loadingGameDescriptionId === viewingGame?.id;
  ...
  function chooseCandidate(candidate: BoardlifeSearchResult) {
    ...
    startDetailTransition(async () => {
      try {
        ...
        form.setFieldsValue({ ...fallback, tags: [], personalRating: 0, recommendationWeight: 1, bgtiWeights: estimateBgtiWeights({ tags: [], complexity: fallback.complexity, playTime: fallback.playTime }), review: "", plays: 0, status: "owned", videos: [], createdAt: new Date().toISOString() });
  ...
  function viewGameDetail(game: CollectionGame, options: { replaceHistory?: boolean } = {}) {
    ...
    setLoadingGameDescriptionId(game.id);
    void fetch(`/api/games/${game.id}/description`)
      .then(async (response) => response.ok ? response.json() as Promise<{ description?: string | null; source?: "boardlife" | "boardgamegeek" | "metadata"; updatedAt?: string }> : null)
      .then((result) => {
        const description = result?.description ?? null;
        if (result?.source === "metadata") {
          descriptionCacheRef.current.set(game.id, null);
          return;
        }
      ...
      .finally(() => {
        ...
        setLoadingGameDescriptionId((current) => current === game.id ? null : current);
      });
  }
  ...
  async function searchRelatedVideos(game: Pick<BoardGameMetadata, "title" | "englishTitle">, autoAdd: boolean, pageToken?: string, page = 1) {
    ...
    try {
      ...
      const response = await fetch(`/api/youtube/search?${searchParams}`, pageToken ? { cache: "no-store" } : undefined);
      const result = await response.json() as { videos?: GameVideo[]; nextPageToken?: string; message?: string };
  ...
  async function addManualVideo() {
    ...
    try {
      const response = await fetch(`/api/youtube/video?youtubeId=${encodeURIComponent(youtubeId)}`, { cache: "no-store" });
      const video = await response.json() as GameVideo | { message?: string };
      if (!response.ok || !("youtubeId" in video)) throw new Error("message" in video ? video.message : "YouTube 영상 정보를 가져오지 못했습니다.");
      form.setFieldValue("videos", [...selectedVideos, video]);
      setManualVideoUrl("");
    } catch (error) {
      messageApi.error(error instanceof Error ? error.message : "YouTube 영상 정보를 가져오지 못했습니다.");
    } finally {
  ...
  function applyAutoBgtiWeights() {
    form.setFieldValue("bgtiWeights", estimateBgtiWeights({
      tags: form.getFieldValue("tags") ?? [],
      complexity: form.getFieldValue("complexity"),
      playTime: form.getFieldValue("playTime"),
    }));
    messageApi.success("태그와 난이도를 기준으로 BGTI 가중치를 다시 계산했습니다.");
  }
  ...
  const profileMenuItems = [
    ...
    ...(!isAdmin ? [{ key: "login", icon: <LockOutlined />, label: "관리자 로그인" }] : []),
              ...
              <Row gutter={[20, 20]}>
                {activeMenu !== "registration" && <Col xs={24} xl={24}>
                  <div hidden={!shouldKeepHomeMounted} className={`home-page ${isHome ? "is-active" : "is-preserved"}`} aria-hidden={!isHome}>
                    ...
                    {!isHomeVisualLoading && popularTagGroups.length > 0 && <HomeTagRail groups={popularTagGroups} onTag={(tag, games) => tag && setTagGallery({ tag, games, sortLabel: "분류 태그와 인원·난이도·플레이 시간 적합도순" })} />}
                  </div>
                  ...
                  <div hidden={activeMenu !== "collection"}><Card id="collection" className="section-card collection-card" title={<Tabs className="collection-status-tabs" activeKey={collectionStatusFilter} onChange={(key) => setCollectionStatusFilter(key as CollectionGame["status"])} items={(Object.keys(statusLabel) as CollectionGame["status"][]).map((status) => ({ key: status, label: statusLabel[status] }))} />}>
                    ...
                    {!filteredCollection.length && <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={!collection.length ? "아직 등록한 보드게임이 없습니다." : hasGamesInSelectedCollectionStatus ? "검색 결과가 없습니다." : `${statusLabel[collectionStatusFilter]} 게임이 없습니다.`} />}
                  </Card></div>
                  ...
                  <div hidden={activeMenu !== "bgti"}><BgtiGuide /></div>
